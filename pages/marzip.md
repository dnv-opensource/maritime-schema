...
# Marzip: Ship Traffic Timeseries Data Format

## Introduction

This page describes marzip, a file format for ship-based, time-series
data. Its primary purpose is to efficiently store result data generated
by collision and grounding avoidance systems. However, it can be used to
store any time-series data relating to ships moving over time.

## FORMAT

As the number of data-points generated can be large, the data is stored
in [Apache Arrow](https://arrow.apache.org/) tables. These can be stored
as binary files, providing a good compromise between serialization /
deserialization speed and file size. The following 2 tables are
described in this document:

1.  timeseries.arrow: This arrow table contains timeseries data relating
    to ships.

2.  shipstatic.arrow: This arrow table contains static data about each
    ship.
...
A result file is stored as a marzip file. This is a normal zip file but
uses a custom extension to indicate the data stored is from the maritime
domain. Both arrow tables shall be placed in the zip file and named
timeseries.arrow and shipstatic.arrow.

To compress the zip file, DEFLATE compression, or no compression (STORE)
can be used.
